...
class CalculateAverageDay:
    ...
    def Run(self):
        deltaObjectList =[]
        with open('TestDayAverageVariables.json','r+') as outfile:
            json_data = json.load(outfile) 
        self.from_string_list = json_data['From Statuses']
        self.to_string_list = json_data['To Statuses']
        self.USERNAME = json_data['Username']
        self.APIKEY =  bytes(json_data['ApiKey'],'utf-8')
        self.JQL_query= json_data['JQL Query']
        self.options = json_data['Server Options']
        last_reported_average = json_data['Last Reported Average']
        last_reported_delta = json_data['Last Reported Average Delta']
        average_delta = json_data['Average Delta']
        ...
        self.update_json_file('TestDayAverageVariables.json',json_data,average_days,last_reported_average,last_reported_delta,average_delta,self.total_sum_of_days,sortedDeltaObjectDictionary)
        jira_client.close()
